...
import { rateARecipe } from "@/actions/recipe";
...
const RatingPopUp = ({
  ...
  const onSubmit = async (val: number) => {
    setRating(val);
    setOpen(false);
    await rateARecipe(recipeId, val);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>How do you feel about this recipe?</DialogTitle>
          <DialogDescription>
            Describe your experience with this recipe. Your feedback will help
            us improve our recipes.
          </DialogDescription>
        </DialogHeader>
        <div className="flex justify-center items-center">
          <Rating
            style={{ maxWidth: 300 }}
            value={rating}
            onChange={(val: number) => onSubmit(val)}
